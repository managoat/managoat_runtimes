...
> How a coding agent CLI (claude, codex, gemini, opencode) gets into a
> sandbox and comes up speaking ACP.
...
```
lib/                    13 modules — README.md is the guide to them
test/                   13 test files, mirroring lib/
...
package, not of this suite: `Managoat.Runtimes.Testing.FakeRuntime`.
...
The behaviour-focused suite drives the runtimes' bootstraps, adapter install,
model translation and the skills mechanism against a stubbed sandbox. The
remaining misses are defensive or timing-only paths, so there is a little
headroom for a real branch.
...
`test/test_helper.exs` copies the sandbox seam for Mimic:

```elixir
Mimic.copy(Managoat.Sandbox)          # the facade
Mimic.copy(Managoat.Sandbox.Sprites)  # the adapter behind it
```

Both, because some tests expect on the facade and some on the adapter under it.
Nothing else is configured: the library reads no configuration, and the fake
runtime's observer is set per test by whoever uses it.

**`priv/` ships.** `Managoat.Runtimes.Gemini.SessionStore` embeds it at compile
time, so `priv` is on the `files:` line in `mix.exs` — which also means a change
under `priv/` is a change to what the package ships and needs a release.
...
- **Don't unpin an ACP adapter version.** The versions in
  `Managoat.Runtimes.ACP` are exact, and that is load-bearing: an unpinned
  adapter can stop advertising `sessionCapabilities.resume` in a point release
...
- **Don't add a per-runtime workaround without its deletion condition.**
  ...
  goes away; a workaround with no exit is a permanent one.
